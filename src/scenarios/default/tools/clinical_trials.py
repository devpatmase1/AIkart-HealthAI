...
class ClinicalTrialsPlugin:
    def __init__(self, kernel: Kernel, chat_ctx: ChatContext, app_ctx: AppContext):
        ...
        if is_gemini or is_groq or (has_openai and not has_azure):
            ...
            reasoning_deployment = os.getenv("AZURE_OPENAI_DEPLOYMENT_NAME_REASONING_MODEL") or os.getenv("AZURE_OPENAI_DEPLOYMENT_NAME", "gpt-4o")
            reasoning_endpoint = os.getenv("AZURE_OPENAI_REASONING_MODEL_ENDPOINT") or os.getenv("AZURE_OPENAI_ENDPOINT", "")
            azure_kwargs = {
                "service_id": "reasoning-model",
            ...
            if reasoning_endpoint:
                azure_kwargs["endpoint"] = reasoning_endpoint
            if os.getenv("AZURE_OPENAI_API_KEY"):
                azure_kwargs["api_key"] = os.getenv("AZURE_OPENAI_API_KEY")
            elif hasattr(self.app_ctx, "cognitive_services_token_provider"):
                try:
                    azure_kwargs["ad_token_provider"] = self.app_ctx.cognitive_services_token_provider
                except Exception:
                    pass
            self.chat_completion_service = AzureChatCompletion(**azure_kwargs)
    ...
    async def search_clinical_trials(self, clinical_trials_query: str, age: str, biomarkers: list[str], histology: str, staging: str, ecog_performance_status: str, first_line_treatment: str, second_line_treatment: str) -> str:
        ...
        logger.info(f"Clinical trials found: {len(studies)}")

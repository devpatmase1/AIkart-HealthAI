...
def user_routes():
    ...
    async def get_current_user(request: Request):
        ...
        try:
            ...
            # If we don't have a user ID, check if running locally
            if not user_info["id"]:
                if os.getenv("WEBSITE_SITE_NAME") is None:
                    user_info = {
                        "id": "local-physician-1",
                        "name": "Dr. Local Physician",
                        "email": "physician@hospital.local",
                        "roles": ["Physician", "TumorBoardMember"]
                    }
                else:

...
def chats_routes(app_context: AppContext):
    ...
    async def websocket_chat_endpoint(websocket: WebSocket, chat_id: str):
        ...
        try:
            ...
            # Get responses from the target agent with retries for rate limits
            max_invocation_retries = 3
            for attempt in range(max_invocation_retries):
                try:
                    ...
                    if ("429" in str(e) or "rate_limit" in str(e).lower()) and attempt < max_invocation_retries - 1:
                        backoff = 3 * (attempt + 1)
                        logger.warning(f"Rate limit encountered during chat.invoke. Retrying in {backoff}s... (Attempt {attempt + 1}/{max_invocation_retries})")
